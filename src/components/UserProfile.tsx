...
export default function UserProfile() {
  ...
  const {userName, userInfo } = readContract(); // Custom hook to read contract data (owner address and paused status)
  
  
  const [addUserData, setAddUserData] = useState({ username: '', name: '', email: '' });
  
  const { addUserDetails, data: hash, isPending } = writeContract();
    ...
    <div className="bg-gray-800/40 backdrop-blur-sm p-6 md:p-10 rounded-3xl border border-gray-700 shadow-2xl transition-all">
      ...
      {!isRegistered ? (
        <div className="space-y-5 max-w-lg">
          ...
          <button 
            onClick={() => addUserDetails(addUserData.username, addUserData.name, addUserData.email)}
        ...
        <div className="bg-gray-900/60 border border-gray-700 p-8 rounded-2xl w-full shadow-inner">
